...
bool ConnInfoUI::LoadConfig( const std::string& path )
{
    ...
    for (Value::ConstValueIterator itr = serverConfig.Begin(); itr != serverConfig.End(); ++itr)
    {
        ...
        m_dicServerInfo[kServerNameIndex].push_back(Base::CharacterSet::UnicodeToUTF8(name.GetData()));
...
bool ConnInfoUI::SaveConfig( const std::string& path )
{
    ...
    for (std::size_t idx=0; idx<theSize; ++idx)
    {
        writer.StartObject();
        writer.String("name");
        CDuiString name = Base::CharacterSet::UTF8ToUnicode(m_dicServerInfo[kServerNameIndex][idx]).c_str();
...
LPCTSTR ConnInfoUI::GetItemText( CControlUI* pControl, int iIndex, int iSubItem )
{
    ...
    if (iSubItem == 3 && !str.empty()) 
    {
        ...
        pControl->SetUserData(Base::CharacterSet::UTF8ToUnicode(str).c_str());
    }
...
void ConnInfoUI::OnItemActive( TNotifyUI &msg )
{
    ...
    CDuiString name = CDuiString(Base::CharacterSet::UTF8ToUnicode(m_dicServerInfo[kServerNameIndex][idx]).c_str());
