...
void RedisConfigUI::DoFillData()
{
    ...
    for ( ; it!=itend; ++it)
    {
        ...
        CDuiString key = Base::CharacterSet::UTF8ToUnicode(it->first).c_str();
        CDuiString val = Base::CharacterSet::UTF8ToUnicode(it->second).c_str();
        name.Append(key);
...
void RedisConfigUI::DoApplyData()
{
    ...
    for ( ; it!=itend; ++it)
    {
        ...
        {
            CEditUI* pEdit = static_cast<CEditUI*> (pCtl);
            val = Base::CharacterSet::UnicodeToUTF8(pEdit->GetText().GetData());
        }
        else if (pCtl->GetClass() == exampleComboUI.GetClass())
        {
            CComboUI* pCombo = static_cast<CComboUI*> (pCtl);
