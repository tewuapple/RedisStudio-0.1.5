...
void RedisDataUI::OnItemActiveForTree( TNotifyUI &msg )
{
    ...
    GetResult().Clear();    
    std::string key = Base::CharacterSet::UnicodeToUTF8(pActiveNode->GetItemText().GetData());
    m_pKeyEdit->SetText(Base::CharacterSet::UTF8ToUnicode(key).c_str());
...
void RedisDataUI::OnItemActiveForList(  TNotifyUI &msg  )
{
    ...
    std::size_t curPage = atoi(Base::CharacterSet::UnicodeToUTF8(m_pPageCur->GetText().GetData()).c_str());
    ...
    std::string text = Base::CharacterSet::UnicodeToUTF8(myDuiStr.GetData());
...
{    
    ...
    std::string curPageStr = Base::CharacterSet::UnicodeToUTF8(m_pPageCur->GetText().GetData());
    int curPage = atoi(curPageStr.c_str());
    int idx = (curPage-1)*m_iPageSize + m_pList->GetCurSel();
    std::string key = Base::CharacterSet::UnicodeToUTF8(m_pKeyEdit->GetText().GetData()); 
    std::string oldValue = Base::CharacterSet::UnicodeToUTF8(textElement->GetText(0));
    std::string field;
    if (m_pList->GetHeader()->GetCount() > 1)
    {        
        field = Base::CharacterSet::UnicodeToUTF8(textElement->GetText(0));
        oldValue = Base::CharacterSet::UnicodeToUTF8(textElement->GetText(1));
    }
    std::string newValue = Base::CharacterSet::UnicodeToUTF8(m_pRichEdit->GetText().GetData()); 
    ...
    {       
        /// todo 此处应该提示错误详情
        CDuiString err(Base::CharacterSet::UTF8ToUnicode(Env()->GetDBClient()->GetLastError()).c_str());
...
void RedisDataUI::BackgroudWorkForRenderLevel(void)
{
    ...
    else 
    {
        ...
        while (pPNode)
        {
            ...
            std::string key = Base::CharacterSet::UnicodeToUTF8(pPNode->GetItemText().GetData());
            vec.push_back(key);
...
void RedisDataUI::BackgroundWorkForRefreshValues(void)
{
    ...
    key = Base::CharacterSet::UnicodeToUTF8(pKeyEdit->GetText().GetData());
    if (!cli->Exists(key))
    {
        /// 如果该键已经不存在，则提示用户重新加载
    ...
    m_oRedisData.key = Base::CharacterSet::UTF8ToUnicode(key).c_str();

    if (!cli->GetData(key, type, GetResult())) return;
    m_oRedisData.type = Base::CharacterSet::UTF8ToUnicode(type).c_str();
    ...
    if (ttl > 0) 
    {
        ...
        m_oRedisData.ttl.Append(_T("("));
        m_oRedisData.ttl.Append(Base::CharacterSet::UTF8ToUnicode(Base::Util::ConvertHumanTimeDuration(ttl)).c_str());	
        m_oRedisData.ttl.Append(_T(")"));
    } 
    /// 预先显示数据类型等信息
    ...
    for (std::size_t idx=0; idx<GetResult().ColumnSize(); ++idx)
    {
        ...
        pHeaderItem->SetText(Base::CharacterSet::UTF8ToUnicode(GetResult().ColumnName(idx)).c_str());
...
void RedisDataUI::SetPageValues( )
{
    ...
    std::string pageStr = Base::CharacterSet::UnicodeToUTF8(m_pPageCur->GetText().GetData());
...
LRESULT RedisDataUI::OnKeyDel( HWND hwnd, WPARAM wParam, LPARAM lParam )
{
    ...
    std::string key = Base::CharacterSet::UnicodeToUTF8(pNode->GetItemText().GetData());
    /// 暂时用同步方法删除，大数据时会有卡顿
    DBClient* cli = Env()->GetDBClient();
...
void RedisDataUI::SetRichEditText(const std::string& text)
{
    ...
    m_pRichEdit->SetText(Base::CharacterSet::UTF8ToUnicode(styleText).c_str());
}
...
CTreeNodeUI* RedisDataUI::NewNode(const string& text, bool isLeaf)
{
    ...
    pNodeTmp->SetItemText(Base::CharacterSet::UTF8ToUnicode(text).c_str());  
